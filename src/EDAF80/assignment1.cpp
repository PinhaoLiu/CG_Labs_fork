...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) {
		...
		{
			// Iterative approach.
			auto s = new std::stack<CelestialBodyRef>(); // create a stack
			s->push({ &sun, glm::translate(glm::mat4(1.0f), glm::vec3(2.0f, 0.0f, 0.0f)) }); // initialize the first element

			// while the stack not empty
			while (!s->empty())
			{
				auto current = s->top(); // take the first element from stack
				s->pop(); // delete the first element from stack

				auto body = current.body; // celestial body
				auto pt = current.parent_transform; // parent transform

				// render celestial body
				auto transform = body->render(
				...
				// add current celestial body's all children into stack
				for (auto iter : body->get_children())
					s->push({ iter, transform });
			}
