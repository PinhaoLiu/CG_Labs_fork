	...
	unsigned int const latitude_split_count)
{
	...
	size_t index = 0u;
	float theta = 0.0f;
	for (unsigned int i = 0u; i < longitude_split_vertices_count; ++i) {
		float const cos_theta = std::cos(theta);
		float const sin_theta = std::sin(theta);

		float phi = 0.0f;
		for (unsigned int j = 0u; j < latitude_split_vertices_count; ++j) {
			...
			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(i) / (static_cast<float>(latitude_split_vertices_count)),
				static_cast<float>(j) / (static_cast<float>(longitude_split_vertices_count)),
				0.0f);

			// tangent
			auto const t = phi != 0.0f ? glm::vec3(radius * cos_theta * sin_phi,
				0.0f,
				-radius * sin_theta * sin_phi)
				: glm::vec3(radius * cos_theta * 0.01f,
					0.0f,
					-radius * sin_theta * 0.01f);
			tangents[index] = t;
	...
	index = 0u;
	for (unsigned int i = 0u; i < longitude_split_edges_count; ++i)
	{
		for (unsigned int j = 0u; j < latitude_split_edges_count; ++j)
		{
			index_sets[index] = glm::uvec3(latitude_split_vertices_count * (i + 0u) + (j + 0u),
				latitude_split_vertices_count * (i + 0u) + (j + 1u),
				latitude_split_vertices_count * (i + 1u) + (j + 1u));
			++index;

			index_sets[index] = glm::uvec3(latitude_split_vertices_count * (i + 0u) + (j + 0u),
				latitude_split_vertices_count * (i + 1u) + (j + 1u),
				latitude_split_vertices_count * (i + 1u) + (j + 0u));
			++index;
		}
	}
	...
	unsigned int const minor_split_count)
{
	...
	float const d_phi = glm::two_pi<float>() / (static_cast<float>(major_split_edges_count));
	float const d_theta = glm::two_pi<float>() / (static_cast<float>(minor_split_edges_count));
...
bonobo::mesh_data
parametric_shapes::createQuadWaterWave(float const width, float const height,
	unsigned int const horizontal_split_count,
	unsigned int const vertical_split_count)
{
	...
	float const delta_x = width / (static_cast<float>(horizontal_split_edges_count));
	float const delta_z = height / (static_cast<float>(vertical_split_edges_count));

	//generate vevrtices iteratively
	size_t index = 0u;
	float x = 0.0f;
	for (unsigned int i = 0u; i < horizontal_split_vertices_count; ++i) {

		float z = 0.0f;
		for (unsigned int j = 0u; j < vertical_split_vertices_count; ++j) {
			// vertex
			vertices[index] = glm::vec3(x, 0.0f, z);

			// texture coordinates
			texcoords[index] = glm::vec3(static_cast<float>(j) / (static_cast<float>(vertical_split_vertices_count)),
				static_cast<float>(i) / (static_cast<float>(horizontal_split_vertices_count)),
				0.0f
			);
	...
	// create index array
	auto index_sets = std::vector<glm::uvec3>(2u * horizontal_split_edges_count * vertical_split_edges_count);

	// generate indeces iteratively
	index = 0u;
	for (unsigned int i = 0u; i < horizontal_split_edges_count; ++i)
	{
		for (unsigned int j = 0u; j < vertical_split_edges_count; ++j)
		{
			index_sets[index] = glm::uvec3(vertical_split_vertices_count * (i + 0u) + (j + 0u),
				vertical_split_vertices_count * (i + 0u) + (j + 1u),
				vertical_split_vertices_count * (i + 1u) + (j + 1u));
			++index;

			index_sets[index] = glm::uvec3(vertical_split_vertices_count * (i + 0u) + (j + 0u),
				vertical_split_vertices_count * (i + 1u) + (j + 1u),
				vertical_split_vertices_count * (i + 1u) + (j + 0u));
			++index;
		}
	}
